...
namespace CarsDB
{
    public partial class Contract : Form
    {
        ...
        {
            // TODO: данная строка кода позволяет загрузить данные в таблицу "carsDataSet.Договор". При необходимости она может быть перемещена или удалена.
            this.договорTableAdapter.Fill(this.carsDataSet.Договор);

        }
        ContractsCreate contracts;
        private void button5_Click(object sender, EventArgs e)
        {
            contracts = new ContractsCreate();
        ...
        private void button3_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                try
                {
                    string query = "SELECT Код_Договора FROM Договор WHERE Код_Договора = " + kod;
                    OleDbCommand command = new OleDbCommand(query, myConnection);
                    OleDbDataReader reader = command.ExecuteReader();
                    reader.Read();
                    int count = (int)reader["Код_Договора"];
                    change = new ContractsChange(kod);
                    change.ShowDialog();
                }
                catch
                {
